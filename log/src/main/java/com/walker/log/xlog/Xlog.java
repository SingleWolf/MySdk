...
public class Xlog implements Log.LogImp {
    ...
    public static void open(Options options) {
        if (options == null) {
            throw new RuntimeException("options cannot null");
        }
        if (options.isLoadLib()) {
            loadLib();
        }
        setConsoleLogOpen(options.isConsoleLogOpen());
        if (0 < options.getMaxFileSize()) {
            setMaxFileSize(options.getMaxFileSize());
        }
        if (0 < options.getMaxAliveTime()) {
            setMaxAliveTime(options.getMaxAliveTime());
        }
        appenderOpen(options.getLevel(), options.getMode(), options.getCacheDir(), options.getLogDir(), options.getNamePrefix(), options.getCacheDays(), options.getPubkey());
    }

    private static void loadLib() {
        System.loadLibrary("c++_shared");
        System.loadLibrary("safelog");
    }
